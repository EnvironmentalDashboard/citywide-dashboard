...
" />
  ...
  <g id="clickables">
    <?php
    foreach ($db->query("SELECT component, pos, widthxheight, img, attr FROM cwd_landscape_components WHERE hidden = 0 AND user_id = {$user_id} ORDER BY `order` ASC") as $row) {
  ...
  <script type="text/javascript">
    ...
    <?php if ($admin) { ?>
      ...
      var y_circle_interval = null;
      $('#y-axis-circle').on('mousedown', function(e) {
        if (y_circle_interval === null) {
          var circle = $(this);
          y_circle_interval = setInterval(function() {
            circle.attr('cy', cursorY*1.25);
            if (controllable_component !== null) {
              controllable_component.attr('y', cursorY);
            }
          }, 50);
        }
      }).on('mouseup', function() {
        clearInterval(y_circle_interval);
        y_circle_interval = null;
      });
      var x_circle_interval = null;
      $('#x-axis-circle').on('mousedown', function(e) {
        if (x_circle_interval === null) {
          var circle = $(this);
          x_circle_interval = setInterval(function() {
            circle.attr('cx', cursorX*1.25);
            if (controllable_component !== null) {
              controllable_component.attr('x', cursorX*1.25);
            }
          }, 50);
        }
      }).on('mouseup', function() {
        clearInterval(x_circle_interval);
        x_circle_interval = null;
      });

      $('#<?php echo implode(', #', $components); ?>').on('dblclick', function() {
        if (controllable_component === null) {
          controllable_component = $(this);
          $('#x-axis-circle').attr('cx', controllable_component.attr('x'));
          $('#y-axis-circle').attr('cy', controllable_component.attr('y'));
        } else {
          var x = controllable_component.attr('x');
          var y = controllable_component.attr('y');
          $.post('includes/update-landscape-comp.php', {x: x, y: y, comp: controllable_component.attr('id'), id: <?php echo $user_id ?>});
          controllable_component = null;
          alert("Position saved");
        }
        // cancel showing message box
        // var comp = $(this);
        // comp_id = comp.attr('id');
        // $('#' + comp_id + '_message').attr('display', 'none');
        // $('#' + comp_id + '_close').attr('display', 'none');

        // var w = prompt('Width', comp.attr('width'));
        // var h = prompt('Height', comp.attr('height'));
        // var x = prompt('X coordinate', comp.attr('x'));
        // var y = prompt('Y coordinate', comp.attr('y'));
        // if (w != null && h != null && x != null && y != null) {
        //   $.post('includes/update-landscape-comp.php', {w: w, h: h, x: x, y: y, comp: comp_id, id: <?php //echo $user_id ?>}, function() {
        //     comp.attr('width', w);
        //     comp.attr('height', h);
        //     comp.attr('x', x);
        //     comp.attr('y', y);
