...
require '../includes/class.Gauge.php'; // Some animations depend on the reading of a meter
...
foreach ($db->query('SELECT * FROM cwd_states WHERE `on` = 1') as $row) {
  ...
  $gauge1 = gaugeURL($gauge1_data['meter_uuid'], $gauge1_data['data_interval'], $gauge1_data['color'], $gauge1_data['bg'], $gauge1_data['height'], $gauge1_data['width'], $gauge1_data['font_family'], $gauge1_data['title'], $gauge1_data['title2'], $gauge1_data['border_radius'], $gauge1_data['rounding'], $gauge1_data['ver'], $gauge1_data['units'], $gauge1_data['start']);
  $gauge2_data = $db->query('SELECT * FROM gauges WHERE id = \'' . $row['gauge2'] . '\'')->fetch();
  $gauge2 = gaugeURL($gauge2_data['meter_uuid'], $gauge2_data['data_interval'], $gauge2_data['color'], $gauge2_data['bg'], $gauge2_data['height'], $gauge2_data['width'], $gauge2_data['font_family'], $gauge2_data['title'], $gauge2_data['title2'], $gauge2_data['border_radius'], $gauge2_data['rounding'], $gauge2_data['ver'], $gauge2_data['units'], $gauge2_data['start']);
  $gauge3_data = $db->query('SELECT * FROM gauges WHERE id = \'' . $row['gauge3'] . '\'')->fetch();
  $gauge3 = gaugeURL($gauge3_data['meter_uuid'], $gauge3_data['data_interval'], $gauge3_data['color'], $gauge3_data['bg'], $gauge3_data['height'], $gauge3_data['width'], $gauge3_data['font_family'], $gauge3_data['title'], $gauge3_data['title2'], $gauge3_data['border_radius'], $gauge3_data['rounding'], $gauge3_data['ver'], $gauge3_data['units'], $gauge3_data['start']);
  $gauge4_data = $db->query('SELECT * FROM gauges WHERE id = \'' . $row['gauge4'] . '\'')->fetch();
  $gauge4 = gaugeURL($gauge4_data['meter_uuid'], $gauge4_data['data_interval'], $gauge4_data['color'], $gauge4_data['bg'], $gauge4_data['height'], $gauge4_data['width'], $gauge4_data['font_family'], $gauge4_data['title'], $gauge4_data['title2'], $gauge4_data['border_radius'], $gauge4_data['rounding'], $gauge4_data['ver'], $gauge4_data['units'], $gauge4_data['start']);
...
" />
    ...
    foreach ($db->query('SELECT component, pos, widthxheight, img FROM cwd_landscape_components WHERE hidden = 0 ORDER BY `order` ASC') as $row) {
      if ($row['component'] !== 'river_click') {
        ...
        $wh = explode('x', str_replace(' ', '', strtolower($row['widthxheight'])));
          echo "<image style='cursor:pointer' overflow='visible' enable-background='new    '
  ...
  <g id="landscape_messages"><!-- See #clickables -->
    ...
    <g id='<?php echo $row['component']; ?>_message' display="none">
      <rect width="320" height="<?php echo (strlen($row['text']) * 0.7) + 80 ?>"
            x="<?php echo $explode[0]; ?>"
            y="<?php echo $explode[1]; ?>"
            style="fill:#fff;stroke-width:1;stroke:#333;" />
      <text x="<?php echo $explode[0] + 10; ?>" y="<?php echo $explode[1] + 25; ?>" fill="#333" font-size="22" font-family="Futura, sans-serif"><?php echo $row['title']; ?></text>
      <text x="<?php echo $explode[0] + 10; ?>" y="<?php echo $explode[1] + 60; ?>" fill="#333" font-family="Futura, sans-serif">
        <tspan y="<?php echo $y; ?>">
        <?php
        // Split into words
        ...
        for ($i = 0; $i < count($text); $i++) {
          ...
          if ($num_chars > 25) { // Assumes every charachter is the same size but this usually gets close
            $y += 22;
            echo "</tspan><tspan y='{$y}' x='".($explode[0]+10)."'>";
            $num_chars = 0;
          }
        }
        ?>
        </tspan>
        <tspan style="fill:#3498db;text-decoration:underline;font-size:18px" x="<?php echo $explode[0] + 10; ?>" y="<?php echo $y + 30; ?>"><?php echo "<a style='fill:#3498db;' x='".($explode[0]+10)."' xlink:href='{$row['link']}'>Read more</a>" ?></tspan>
      </text>
    </g>
      <image display="none"
             x="<?php echo $explode[0]+295; ?>"
             y="<?php echo $explode[1]+5; ?>"
             width="20" height="20"
  ...
  <script type="text/javascript">
    ...
    <?php if ($electricity_bool) {echo 'var electricity_messages = ' . json_encode($electricity_messages) . ';';} ?>
    <?php if ($gas_bool) {echo 'var gas_messages = ' . json_encode($gas_messages) . ';';} ?>
    <?php if ($stream_bool) {echo 'var stream_messages = ' . json_encode($stream_messages) . ';';} ?>
    ...
    <?php } if (in_array('water', $resources)) { ?>
    function water() {
    ...
    <?php } if (in_array('stream', $resources)) { ?>
    function stream() {
    ...
    <?php } if (in_array('weather', $resources)) { ?>
    function weather() {
    ...
    // Rain animation
    <?php if ($its_raining) { for ($i = 0; $i < 100; $i++) { ?>
      TweenMax.to($('#raindrop-<?php echo $i; ?>'), 1, {y: 1000, repeat: -1, delay: <?php echo mt_rand(0, 100)/10; ?>});
    <?php } } ?>
    // Ship animation
    TweenMax.to($('#ship'), 60, {scaleX:0.7, scaleY:0.7, x:"1260px", y:"140px", ease:Power1.easeInOut, repeat: -1, repeatDelay: 1});
    // Wind turbine animation
    TweenMax.to($('#blades'), 2.5, {rotation:360, transformOrigin:"50% 60%", repeat: -1, ease:Power0.easeNone});
    ...
    tl1.to(waterlines_clip1, <?php echo $water_speed * 0.25; ?>, {y:80, ease:Power0.easeNone})
    .to(waterlines_clip1, <?php echo $water_speed * 0.25; ?>, {x:50, ease:Power0.easeNone})
    .to(waterlines_clip1, <?php echo $water_speed * 1.5 ?>, {y:385,x:240, ease:Power0.easeNone})
    .to(waterlines_clip1, <?php echo $water_speed * 0.25; ?>, {y:410,x:210, ease:Power0.easeNone})
    .to(waterlines_clip1, <?php echo $water_speed; ?>, {y:450,x:-100, ease:Power0.easeNone});
    var tl2 = new TimelineMax({repeat: -1, repeatDelay: 2});
    var waterlines_clip2 = $('#waterline_clip1').children()[1];
    tl2.to(waterlines_clip2, <?php echo $water_speed; ?>, {x:230, y:140, ease:Power0.easeNone});
    var tl3 = new TimelineMax({repeat: -1, repeatDelay: 2});
    var waterlines_clip3 = $('#waterline_clip1').children()[2];
    tl3.to(waterlines_clip3, <?php echo $water_speed; ?>, {x:-230, y:160, ease:Power0.easeNone});
    var tl4 = new TimelineMax({repeat: -1});
    var waterlines_clip4 = $('#waterline_clip2').children()[0];
    tl4.to(waterlines_clip4, <?php echo $water_speed; ?>, {x:-330, y:75, ease:Power0.easeNone});
    var tl5 = new TimelineMax({repeat: -1});
    var waterlines_clip5 = $('#waterline_clip3').children()[0];
    tl5.to(waterlines_clip5, <?php echo $water_speed; ?>, {x:170, y:270, ease:Power0.easeNone})
    .to(waterlines_clip5, <?php echo $water_speed * 0.375 ?>, {x:50, y:310, ease:Power0.easeNone})
    .to(waterlines_clip5, <?php echo $water_speed * 1.5 ?>, {x:-500, y:340, ease:Power0.easeNone})
    .to(waterlines_clip5, <?php echo $water_speed * 0.5 ?>, {x:-620, y:230, ease:Power0.easeNone})
    var tl6 = new TimelineMax({repeat: -1, repeatDelay: 2});
    var waterlines_clip6 = $('#waterline_clip3').children()[1];
    tl6.to(waterlines_clip6, <?php echo $water_speed * 0.5 ?>, {x:-300, ease:Power0.easeNone});
    var tl6 = new TimelineMax({repeat: -1, repeatDelay: 0.5});
    var waterlines_clip6 = $('#waterline_clip4').children()[0];
    tl6.to(waterlines_clip6, <?php echo $water_speed * 0.5 ?>, {x:-50, y: 75, ease:Power0.easeNone})
    .to(waterlines_clip6, <?php echo $water_speed * 0.75 ?>, {x:100, y: 150, ease:Power0.easeNone})
    .to(waterlines_clip6, <?php echo $water_speed * 0.5 ?>, {x:0, y: 200, ease:Power0.easeNone});
    // Flow marks animation
    TweenMax.to($('#flow_marks_clip').children()[0], 3, {y:"-600px", ease:Power0.easeNone, repeat: -1});
    // Smoke animation
    TweenMax.to($('#smoke > image'), 1, {y: "-60px", x: "20px", scaleX: 2, scaleY: 1.5, opacity: 0, ease:Power0.easeNone, repeat: -1, repeatDelay: 1.1});
    // Spark animations
    TweenMax.to($("#sparkpaths").children()[0], <?php echo $electricity_speed * 0.5; ?>, {bezier:[{x:15,y:-25},{x:30,y:-50}], ease:Power0.easeNone, repeat: -1});
    TweenMax.to($("#sparkpaths").children()[1], <?php echo $electricity_speed * 0.5; ?>, {bezier:[{x:-150,y:40},{x:-200,y:20}], ease:Power0.easeNone, repeat: -1});
    TweenMax.to($("#sparkpaths").children()[2], <?php echo $electricity_speed * 0.5; ?>, {bezier:[{x:60,y:-70},{x:100,y:-140}], ease:Power0.easeNone, repeat: -1});
    TweenMax.to($("#sparkpaths_back").children()[0], <?php echo $electricity_speed * 0.5; ?>, {bezier:[{x:-10,y:50},{x:-20,y:70}], ease:Power0.easeNone, repeat: -1});
    TweenMax.to($("#sparkpaths_back").children()[1], <?php echo $electricity_speed * 0.5; ?>, {bezier:[{x:25,y:50},{x:75,y:100}], ease:Power0.easeNone, repeat: -1});
